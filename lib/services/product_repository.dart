...
class ProductRepository {
  ...
  /// Import predefined medicines from the bundled JSON seed file.
  /// Returns the number of products inserted.
  static const _seedAssetPath = 'assets/data/medicines_seed.json';
  ...
  Future<int> importMedicines() async {
    ...
    // Load seed data from bundled JSON asset.
    final jsonStr = await rootBundle.loadString(_seedAssetPath);
    final List<dynamic> seedList = jsonDecode(jsonStr) as List<dynamic>;
    final medicines = seedList.cast<Map<String, dynamic>>();
    ...
    String? catGlobalId;
    if (catRow.isEmpty) {
      catGlobalId = const Uuid().v4();
      catId = await db.insert('categories', {
        'name': 'Médicaments',
        'isActive': 1,
        'tenantId': tid,
        'createdAt': now,
        'global_id': catGlobalId,
      });
    } else {
      catId = catRow.first['id'] as int;
      catGlobalId = catRow.first['global_id'] as String?;
    }
    ...
    for (final m in medicines) {
      ...
      final existingCode = m['productCode'] as String?;
      final productCode = (existingCode != null && existingCode.isNotEmpty)
          ? existingCode
      ...
      final productId = await db.insert('products', {
        ...
        'buyPrice': (m['buyPrice'] as num?)?.toInt() ?? 0,
        'lowStockThreshold': (m['lowStockThreshold'] as num?)?.toInt() ?? 5,
        'categoryId': catId,
        'tenantId': tid,
        'trackInventory': (m['trackInventory'] as num?)?.toInt() ?? 1,
        'isActive': 1,
        'createdAt': now,
        'updatedAt': now,
        ...
        'taxPercent': (m['taxPercent'] as num?)?.toInt() ?? 0,
        'isService': (m['isService'] as num?)?.toInt() ?? 0,
        'global_id': const Uuid().v4(),
        'status': m['status'] as String? ?? (qty <= 5 ? 'low' : 'instock'),
      });
      // Enqueue sync mutation so cloud sync picks up this insert.
      await _enqueueProductMutation(db, productId, 'INSERT');
      inserted++;
    }
    if (inserted > 0) {
      CloudSyncService.instance.scheduleSyncSoon();
